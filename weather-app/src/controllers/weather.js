...
const weather = (req, res) => {
    ...
    const getCityCode = async () => {
      ...
      console.log('error in the function getCityCode');
    ...
    const getElements = async () => {
      ...
      if (typeof(apiJsonWeather) === 'object') {
          ...
          res.send(`<h1>City: ${req.city}</h1> <h3>Temperature: ${temperature}</h3> <h3>Condition: ${condition}</h3> <h3>Is it Daylight? ${isDayLight}</h3> <image src="${iconLink}"></image><form><input type="button" value="Go back!" onclick="history.back()"></form>`);
      } else if (typeof(apiJsonWeather) === 'undefined') {
        res.send(`Please, select a valid city's name <br><form><input type="button" value="Go back!" onclick="history.back()"></form>`);
      }}
      getElements();   
}
